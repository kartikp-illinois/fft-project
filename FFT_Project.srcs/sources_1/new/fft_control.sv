...
localparam PIPELINE_DEPTH = 32;  // safe overkill
...
always_ff @(posedge clk) begin
    if (rst) begin
        ...
        case (state)
            ...
            DRAIN_PIPELINE: drain_count <= drain_count + 1;
...
always_comb begin
    ...
    case (state)
        IDLE: begin
            busy = 0;
            if (start) next_state = INIT_STAGE;
        end

        INIT_STAGE: next_state = COMPUTE;

        COMPUTE: begin
            compute_enable = (bf_count < butterflies_per_stage);
            we_mem         = 1;
            if (bf_count >= butterflies_per_stage) next_state = DRAIN_PIPELINE;
        end

        DRAIN_PIPELINE: begin
            we_mem = 1;
            if (drain_count >= PIPELINE_DEPTH) begin
                if (current_stage >= 7) next_state = FINISH;
                else next_state = NEXT_STAGE;
            end
        end

        NEXT_STAGE: next_state = INIT_STAGE;
