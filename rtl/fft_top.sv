...
// Three read-alignment cycles plus four butterfly stages.
localparam ADDR_PIPE_DEPTH = 7;
...
fft_address_gen #(.FFT_SIZE(FFT_SIZE)) addr_gen (
    ...
    .wr_addr_y0(wr_addr_y0), .wr_addr_y1(wr_addr_y1), .valid()
);
...
fft_bram #(.WIDTH(WIDTH), .DEPTH(FFT_SIZE)) data_mem (
    ...
    // Port B (Now R/W)
    .addr_b(mem_addr_b), .we_b(mem_we_b),
    .din_b_re(mem_din_b_re), .din_b_im(mem_din_b_im),
